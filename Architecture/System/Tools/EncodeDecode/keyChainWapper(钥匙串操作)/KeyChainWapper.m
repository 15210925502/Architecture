...
+ (BOOL)addKeyChainWithRSASecKey:(SecKeyRef)SecKey
    ...
    return [self saveQueryKey:queryKey
                    identfier:identifier
                  isPublicKey:isPublickey]?YES:NO;
}
...
+ (SecKeyRef)addKeyChainWithRSAkey:(NSString *)key
    ...
    NSMutableDictionary *queryKey = [self getSecKeyRefKeychainQuery:identifier
...
+ (SecKeyRef)saveQueryKey:(NSDictionary *)dict
                identfier:(NSString *)identifier
              isPublicKey:(BOOL)isPublickey{
...
+ (BOOL)deleteRASKeyWithIdentifier:(NSString *)identifier
    ...
    NSLog(@"删除%@",(int)status == 0 ? @"成功" : @"失败");
    return status == noErr;
}

#pragma mark - 通用方法
+ (NSMutableDictionary *)getSecKeyRefKeychainQuery:(NSString *)identifier
                                       isPublicKey:(BOOL)isPublickey{
...
+ (NSString *)base64EncodedFromPEMFormat:(NSString *)PEMFormat{
    /*
     ...
     -----END RSA PRIVATE KEY----
     */
    NSRange spos = [PEMFormat rangeOfString:@"-----BEGIN PUBLIC KEY-----"];
    NSRange epos = [PEMFormat rangeOfString:@"-----END PUBLIC KEY-----"];
    if(spos.location != NSNotFound && epos.location != NSNotFound){
        NSUInteger s = spos.location + spos.length;
        NSUInteger e = epos.location;
        NSRange range = NSMakeRange(s, e-s);
        PEMFormat = [PEMFormat substringWithRange:range];
    }
