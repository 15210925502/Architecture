...
#pragma mark- 3des加密
+ (NSString *)DES3EncryptString:(NSString *)encryptString
                      keyString:(NSString *)keyString
                       ivString:(NSString *)ivString{
    
    return [self doCipher:encryptString
                keyString:keyString
                 ivString:ivString
...
#pragma mark- 3des解密
+ (NSString *)DES3DecryptString:(NSString *)decryptString
                     keyString:(NSString *)keyString
                      ivString:(NSString *)ivString{
    
    return [self doCipher:decryptString
                keyString:keyString
                 ivString:ivString
                operation:kCCDecrypt];
}

+ (NSString *)doCipher:(NSString *)sText
             keyString:(NSString *)keyString
              ivString:(NSString *)ivString
             operation:(CCOperation)encryptOperation{
    const void *dataIn;
    size_t dataInLength;
    //传递过来的是decrypt 解码
