...
+ (void)replicatorLayer_Wave:(CALayer *)layer{
    CABasicAnimation *scale = [self scaleAnimation:0.8
                                             fromX:1.0
                                             fromY:1.0
                                             fromZ:0.0
                                               toX:0.9
...
//    圆圈数量
    CGFloat instanceCount = 3;
//    圆圈之间的间距
    CGFloat between = 5.0;
    CGFloat maxWidth = 100;
    CGFloat maxHight = 100;
    CGFloat radius = (maxWidth - (instanceCount - 1) * between) / instanceCount;
    
    CAShapeLayer *shapeLayer = [CAShapeLayer layer];
    shapeLayer.frame = CGRectMake(0, (maxHight - radius) / 2, radius, radius);
    shapeLayer.path = [UIBezierPath bezierPathWithOvalInRect:CGRectMake(0, 0, radius, radius)].CGPath;
    shapeLayer.fillColor = [UIColor redColor].CGColor;
    [shapeLayer addAnimation:[self scaleAnimation:0.6
                                            fromX:1.0
                                            fromY:1.0
                                            fromZ:0.0
                                              toX:0.2
                                              toY:0.2
                                              toZ:0.0] forKey:@"scaleAnimation"];
    
    CAReplicatorLayer *replicatorLayer = [CAReplicatorLayer layer];
    replicatorLayer.frame = CGRectMake(0, 0, maxWidth, maxHight);
...
+ (CALayer *)replicatorLayer_Grid{
    ...
    CAAnimationGroup *animationGroup = [CAAnimationGroup animation];
    animationGroup.animations = @[[self scaleAnimation:0.6
                                                 fromX:1.0
                                                 fromY:1.0
                                                 fromZ:0.0
                                                   toX:0.2
                                                   toY:0.2
                                                   toZ:0.0], [self alphaAnimation]];
...
+ (CABasicAnimation *)scaleAnimation:(CFTimeInterval)duration
                               ...
                               fromZ:(CGFloat)fz
                                 ...
                                 toZ:(CGFloat)tz{
    CABasicAnimation *scale = [CABasicAnimation animationWithKeyPath:@"transform"];
    scale.fromValue = [NSValue valueWithCATransform3D:CATransform3DScale(CATransform3DIdentity, fx, fy, fz)];
    scale.toValue = [NSValue valueWithCATransform3D:CATransform3DScale(CATransform3DIdentity, tx, ty, tz)];
