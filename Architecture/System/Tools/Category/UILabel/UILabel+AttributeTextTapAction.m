...
- (id<AttributeTapActionDelegate>)delegate{
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setDelegate:(id<AttributeTapActionDelegate>)delegate{
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches
    ...
    [self getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        if (self.tapBlock) {
            self.tapBlock (string , range , index);
        }
        if (self.delegate && [self.delegate respondsToSelector:@selector(attributeTapReturnString:range:index:)]){
            [self.delegate attributeTapReturnString:string range:range index:index];
        }
        if (self.isTapEffect) {
...
- (BOOL)getTapFrameWithTouchPoint:(CGPoint)point
    ...
    if (self.attributedText.length > range.length) {
        ...
        if ([self.attributedText attribute:NSFontAttributeName atIndex:0 effectiveRange:nil]) {
            ...
            font = [UIFont setFontSizeWithValue:17];
        }
        CGPathRelease(Path);
...
- (void)getRangesWithStrings:(NSArray <NSString *>  *)strings{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[self getStringWithRange:range]];
