...
+ (instancetype)wzx_dictionaryWithObjects:(const id [])objects
    ...
    return [self wzx_dictionaryWithObjects:validObjects
                                   forKeys:validKeys
                                     count:count];
}

- (void)wzx_setObject:(id)object
               forKey:(NSString *)key{
    if(!object){
        @try{
            [self wzx_setObject:object
                         forKey:key];
...
+ (void)exchangeOriginalMethod:(Method)originalMethod
                 withNewMethod:(Method)newMethod{
