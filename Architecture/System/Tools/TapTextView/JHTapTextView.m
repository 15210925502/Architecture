...
- (void)shouldCallback{
    ...
    if (_model && _tapDelegate && [_tapDelegate respondsToSelector:@selector(tapTextView:didClickText:range:)]) {
        [_tapDelegate tapTextView:self
                     didClickText:_model.text
                            range:_model.range];
    }
    if (_highlightedBackgroundColor) {
...
- (void)allRangeOftext:(NSString *)text
                inText:(NSString *)theText
...
- (void)changeTapTextBackgroundColor:(UIColor *)bgcolor
                                type:(NSInteger)type{
    if (bgcolor == nil) {
